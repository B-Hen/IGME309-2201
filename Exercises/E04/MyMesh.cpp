...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	*/
	...
	//the center point of the trianle, aka the top left point
	vector3 center = vector3(0.0f, 0.0f, 0.0f);

	//get the degrees of each angle (radians)
	float angle = (2.0f * PI) / a_nSubdivisions;

	//to keep track of the point before // could just do -angle
	vector3 pointBefore = vector3(a_fRadius * cosf(angle * (a_nSubdivisions - 1)), a_fRadius * sin(angle * (a_nSubdivisions -1)), 0);

	for (int i = 0; i < a_nSubdivisions; i++)
	{
		//get the x and y corridinates of the points // for now the z axis is o 
		//x = rcos(theta)
		//y = rsin(theta);
		float x = a_fRadius * cosf(angle * i);
		float y = a_fRadius * sin(angle * i);

		//make the triangle 
		AddTri(center, pointBefore, vector3(x, y, 0));

		//make a new point before so the circle is actually made 
		pointBefore = vector3(x, y, 0);
	}
