	...
	while (m_bRunning)
	{
		...
		while (m_pWindow->pollEvent(event))
		{
			...
			else if (event.type == sf::Event::KeyReleased)
			{
				...
				{
					if (m_bTriState == false)
					{
						//Becuase I made it a bool I need to pass in an int insteas of three floats
						GLuint ComplementaryColor = glGetUniformLocation(m_uShaderProgramID, "ComplementaryColor");
						glUniform1i(ComplementaryColor, 1);
						m_bTriState = true;
					}
					else if (m_bTriState == true)
					{
						GLuint ComplementaryColor = glGetUniformLocation(m_uShaderProgramID, "ComplementaryColor");
						glUniform1i(ComplementaryColor, 0);
						m_bTriState = false;
					}
				}
			}
